...
Produces publication-quality figures from collected benchmark data.
...
LABELS = ["Tiny Pointer\n(bit-packed)", "Naive\n(full offset)", "Probabilistic\n(key-dep.)", "std::unordered\n_map"]
N_QUERIES = 2_000_000

# ---------------------------------------------------------------------------
# Raw benchmark data (5 repetitions, Google Benchmark, full RockYou 14.3M)
# ---------------------------------------------------------------------------
...
# perf stat: 3 runs averaged, 2M miss queries, full RockYou
# counters: cache-misses, cache-references, instructions, cycles,
#           branch-misses, dtlb-load-misses, l1d.replacement
perf_raw = {
    "tinyptr": {
...
# Average perf counters across 3 runs and derive metrics
perf = {}
...
# Percentile latency: RDTSC, 2M samples, 500K warmup, miss workload
latency = {
    "tinyptr": {"mean": 116.321, "stddev": 131.757, "p50":  91.513, "p95": 186.334, "p99": 424.488, "p999": 1873.259, "max": 15793.154},
    "naive":   {"mean": 114.667, "stddev": 122.385, "p50":  93.718, "p95": 191.846, "p99": 417.321, "p999":  767.937, "max": 40734.849},
    "prob":    {"mean": 111.635, "stddev":  82.141, "p50":  92.064, "p95": 183.577, "p99": 405.744, "p999":  571.680, "max": 18117.914},
    "stdmap":  {"mean": 425.866, "stddev": 735.963, "p50": 335.731, "p95": 868.271, "p99": 5079.521, "p999": 8490.309, "max": 45883.281},
}

# cracker_bench: 3 runs per impl/thread, mid-list target, MH/s
cracker = {
    "tinyptr": {1: [3.06, 3.04, 2.53], 2: [3.21, 3.33, 2.94], 4: [3.61, 3.78, 3.44], 8: [3.67, 3.67, 3.78]},
    "naive":   {1: [2.62, 2.63, 2.52], 2: [3.17, 3.39, 3.32], 4: [3.52, 3.77, 3.73], 8: [3.69, 3.76, 3.33]},
    "prob":    {1: [3.04, 2.63, 3.17], 2: [3.34, 3.25, 3.43], 4: [3.75, 3.70, 3.64], 8: [4.13, 4.02, 4.01]},
    "stdmap":  {1: [2.52, 3.07, 3.03], 2: [3.15, 3.14, 3.06], 4: [3.39, 3.36, 3.34], 8: [3.46, 3.53, 3.45]},
}

# Hyperfine: 5 runs, 2 warmups, 4 threads
hyperfine = {
...
def welch_t_test(a_mean, a_std, a_n, b_mean, b_std, b_n):
    """Welch's t-test for two independent samples given summary statistics."""
...
def add_sig_bracket(ax, x1, x2, y, p, h=0.02):
    """Draw a significance bracket between two bars."""
    label = sig_label(p)
    ax.plot([x1, x1, x2, x2], [y, y + h, y + h, y], lw=1.0, color="#333333")
    ax.text((x1 + x2) / 2, y + h + 0.005, label,
            ha="center", va="bottom", fontsize=7.5, color="#333333")

# ---------------------------------------------------------------------------
# Figure 1: Miss lookup latency (ns/op) -- two panels
# ---------------------------------------------------------------------------

fig, (ax_main, ax_zoom) = plt.subplots(1, 2, figsize=(9, 3.8),
                                        gridspec_kw={"width_ratios": [1, 3]})

means_miss   = [gbench["miss"][i]["mean"]   for i in IMPLS]
stddevs_miss = [gbench["miss"][i]["stddev"] for i in IMPLS]
ci_lo = [m - ci_95(m, s, 5)[0] for m, s in zip(means_miss, stddevs_miss)]
ci_hi = [ci_95(m, s, 5)[1] - m for m, s in zip(means_miss, stddevs_miss)]

bars = ax_main.bar(x, means_miss, color=colors, edgecolor="white", linewidth=0.5)
ax_main.errorbar(x, means_miss, yerr=[ci_lo, ci_hi], fmt="none",
                 capsize=4, ecolor="#333333", elinewidth=1.2, capthick=1.2)
ax_main.set_yscale("log")
ax_main.set_ylabel("Latency (ns/op, log scale)")
ax_main.set_xticks(x)
ax_main.set_xticklabels(LABELS, fontsize=7.5)
ax_main.set_title("All Implementations")
for bar, m in zip(bars, means_miss):
    ax_main.text(bar.get_x() + bar.get_width() / 2, m * 1.8,
                 f"{m:.1f}", ha="center", va="bottom", fontsize=7.5)

zoom_impls  = ["tinyptr", "naive", "prob"]
zoom_labels = [LABELS[IMPLS.index(i)] for i in zoom_impls]
...
xz  = np.arange(len(zoom_impls))

bars2 = ax_zoom.bar(xz, zm, color=zc, edgecolor="white", linewidth=0.5)
ax_zoom.errorbar(xz, zm, yerr=[zci_lo, zci_hi], fmt="none",
                 capsize=4, ecolor="#333333", elinewidth=1.2, capthick=1.2)
ax_zoom.set_ylabel("Latency (ns/op)")
ax_zoom.set_xticks(xz)
ax_zoom.set_xticklabels(zoom_labels, fontsize=8)
ax_zoom.set_title("Custom Implementations (zoomed, 95% CI)")
ax_zoom.set_ylim(0, max(zm) * 1.6)
for bar, m, s in zip(bars2, zm, zs):
    ax_zoom.text(bar.get_x() + bar.get_width() / 2, m + s + 0.3,
                 f"{m:.3f}", ha="center", va="bottom", fontsize=8)

# Significance brackets on zoom panel
...
y_base = max(zm) * 1.25
add_sig_bracket(ax_zoom, 0, 1, y_base,       p_tp_na, h=0.8)
add_sig_bracket(ax_zoom, 0, 2, y_base + 1.8, p_tp_pr, h=0.8)
add_sig_bracket(ax_zoom, 1, 2, y_base + 3.6, p_na_pr, h=0.8)

fig.suptitle("Figure 1: Miss-Query Lookup Latency (Google Benchmark, n=5, 14.3M entries, error bars = 95% CI)",
             fontsize=9, y=1.01)
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/fig1_lookup_latency.pdf")
plt.savefig(f"{OUT_DIR}/fig1_lookup_latency.png")
plt.close()

# ---------------------------------------------------------------------------
# Figure 2: Hit vs Miss vs Mixed latency grouped bar chart
# ---------------------------------------------------------------------------
...
workload_labels = ["Miss", "Hit", "Mixed"]
workload_keys   = ["miss", "hit", "mixed"]
hatches = ["", "//", ".."]

for wi, (wk, wl, hatch) in enumerate(zip(workload_keys, workload_labels, hatches)):
    ...
    xs = x + offsets[wi]
    bars = ax.bar(xs, means, bar_w, color=colors, edgecolor="white",
                  linewidth=0.5, alpha=0.85, hatch=hatch, label=wl)
...
ax.set_title("Figure 2: Lookup Latency by Workload (miss / hit / mixed 95:5)")
ax.legend(title="Workload", loc="upper left")
ax.set_ylim(0, max(gbench["hit"]["stdmap"]["mean"], gbench["miss"]["stdmap"]["mean"]) * 1.35)

plt.tight_layout()
plt.savefig(f"{OUT_DIR}/fig2_workload_comparison.pdf")
plt.savefig(f"{OUT_DIR}/fig2_workload_comparison.png")
plt.close()

# ---------------------------------------------------------------------------
# Figure 3: Cache behavior -- 3 subplots with full counter breakdown
# ---------------------------------------------------------------------------
...
fig.suptitle("Figure 3: Cache Hierarchy Counters (perf stat, avg of 3 runs, 2,000,000 miss queries)",
             fontsize=9, y=1.01)
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/fig3_cache_counters.pdf")
plt.savefig(f"{OUT_DIR}/fig3_cache_counters.png")
plt.close()

# ---------------------------------------------------------------------------
# Figure 4: IPC and branch miss rate
# ---------------------------------------------------------------------------
...
for bar, v in zip(b2, bmiss_vals):
    ax2.text(bar.get_x() + bar.get_width() / 2, v + max(bmiss_vals) * 0.02,
             f"{v:.4f}", ha="center", va="bottom", fontsize=8)

fig.suptitle("Figure 4: CPU Execution Efficiency (perf stat, avg of 3 runs)",
             fontsize=9, y=1.01)
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/fig4_ipc_branch.pdf")
plt.savefig(f"{OUT_DIR}/fig4_ipc_branch.png")
plt.close()

# ---------------------------------------------------------------------------
# Figure 5: Percentile latency CDF
# ---------------------------------------------------------------------------

fig, (ax_cdf, ax_tail) = plt.subplots(1, 2, figsize=(11, 4))

pct_keys  = ["p50", "p95", "p99", "p999"]
pct_vals  = [50, 95, 99, 99.9]
pct_label = ["p50", "p95", "p99", "p99.9"]

for impl, label in zip(IMPLS, LABELS):
    d    = latency[impl]
    ys   = [d["p50"], d["p95"], d["p99"], d["p999"]]
    ax_cdf.plot(pct_vals, ys, marker="o", label=label,
                color=COLORS[impl], linewidth=1.8, markersize=5)

ax_cdf.set_xlabel("Percentile")
ax_cdf.set_ylabel("Latency (ns)")
ax_cdf.set_title("Percentile Latency (all implementations)")
ax_cdf.legend(fontsize=8)
ax_cdf.set_xticks(pct_vals)
ax_cdf.set_xticklabels(pct_label)

# Tail only (p95 and above), log y scale
for impl, label in zip(IMPLS, LABELS):
    d  = latency[impl]
...
fig.suptitle("Figure 5: Latency Percentiles (RDTSC, 2M samples, 500K warmup, miss workload)",
             fontsize=9, y=1.01)
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/fig5_latency_percentiles.pdf")
plt.savefig(f"{OUT_DIR}/fig5_latency_percentiles.png")
plt.close()

# ---------------------------------------------------------------------------
# Figure 6: Thread scaling MH/s
# ---------------------------------------------------------------------------
...
ax.set_title("Figure 6: Cracker Throughput Scaling (error bars = stddev, 3 runs)")
ax.set_xticks(threads)
ax.legend(loc="upper left", ncol=2)
ax.yaxis.set_major_formatter(ticker.FormatStrFormatter("%.2f"))

plt.tight_layout()
plt.savefig(f"{OUT_DIR}/fig6_thread_scaling.pdf")
plt.savefig(f"{OUT_DIR}/fig6_thread_scaling.png")
plt.close()

# ---------------------------------------------------------------------------
# Figure 7: End-to-end wall time (hyperfine) with full min/mean/max
# ---------------------------------------------------------------------------
...
for i, (bar, m, s) in enumerate(zip(bars, hf_means, hf_stds)):
    ax.text(bar.get_x() + bar.get_width() / 2, hf_maxs[i] + 0.15,
            f"{m:.3f}s\n±{s:.3f}s", ha="center", va="bottom", fontsize=7.5)

ax.set_ylabel("Wall-clock time (s)")
ax.set_xticks(x)
ax.set_xticklabels(LABELS, fontsize=8)
ax.set_title("Figure 7: End-to-End Crack Time (hyperfine, 5 runs, 2 warmups, 4 threads)")
ax.legend(fontsize=8)
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/fig7_walltime.pdf")
plt.savefig(f"{OUT_DIR}/fig7_walltime.png")
plt.close()
...
for bar, v in zip(bars, mem_mb):
    ax.text(bar.get_x() + bar.get_width() / 2, v + 10,
            f"{v:.0f} MB", ha="center", va="bottom", fontsize=8.5)
ax.set_ylabel("Peak RSS (MB)")
ax.set_xticks(x)
ax.set_xticklabels(LABELS, fontsize=8)
ax.set_title("Figure 8: Peak Memory Usage (/usr/bin/time -v, 14,344,391 entries)")
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/fig8_memory.pdf")
plt.savefig(f"{OUT_DIR}/fig8_memory.png")
plt.close()

# ---------------------------------------------------------------------------
# Print structured results tables
# ---------------------------------------------------------------------------
...
print("  L1d:       48 KiB x2")
print("  L2:        1280 KiB x2")
print("  L3:        6144 KiB (6 MB)")
...
print("=" * 80)
print("TABLE 1: Lookup Latency -- Miss Workload (Google Benchmark, n=5 reps)")
print("=" * 80)
print(f"{'Impl':<10} {'Mean (ns)':>10} {'Stddev (ns)':>12} {'Median (ns)':>12} {'CV (%)':>8} {'95% CI':>20}")
print("-" * 80)
for impl in IMPLS:
    d  = gbench["miss"][impl]
    lo, hi = ci_95(d["mean"], d["stddev"], 5)
    print(f"{impl:<10} {d['mean']:>10.3f} {d['stddev']:>12.3f} {d['median']:>12.3f} "
          f"{d['cv']:>8.2f} [{lo:.3f}, {hi:.3f}]")

print()
print("TABLE 1b: Lookup Latency -- Hit Workload")
print("-" * 80)
for impl in IMPLS:
    d  = gbench["hit"][impl]
    lo, hi = ci_95(d["mean"], d["stddev"], 5)
...
print()
print("TABLE 1c: Lookup Latency -- Mixed Workload (95% miss / 5% hit)")
print("-" * 80)
for impl in IMPLS:
    d  = gbench["mixed"][impl]
    lo, hi = ci_95(d["mean"], d["stddev"], 5)
...
print()
print("=" * 80)
print("TABLE 2: Statistical Significance -- Miss Latency Pairwise Welch t-tests (n=5)")
print("=" * 80)
print(f"{'Pair':<20} {'t-stat':>8} {'p-value':>10} {'sig':>6} {'Cohen d':>10}")
print("-" * 80)
...
for a, b in pairs:
    ...
    print(f"{a+' vs '+b:<20} {t:>8.3f} {p:>10.4f} {sig_label(p):>6} {d_eff:>10.3f}")

print()
print("=" * 80)
print("TABLE 3: Percentile Latency (RDTSC, 2M samples, overhead-subtracted, ns)")
print("=" * 80)
print(f"{'Impl':<10} {'mean':>9} {'stddev':>9} {'p50':>9} {'p95':>9} {'p99':>9} {'p99.9':>9} {'max':>12}")
print("-" * 80)
for impl in IMPLS:
    d = latency[impl]
...
print()
print("=" * 80)
print("TABLE 4: Hardware Counters (perf stat, avg of 3 runs, 2,000,000 miss queries)")
print("=" * 80)
print(f"{'Impl':<10} {'Cache Misses':>15} {'Cache Refs':>13} {'Miss/Lkup':>11} "
      f"{'L1D/Lkup':>10} {'dTLB/Lkup':>11} {'IPC':>7} {'MissRate':>10}")
print("-" * 80)
for impl in IMPLS:
    d = perf[impl]
    print(f"{impl:<10} {int(d['cache_misses']):>15,} {int(d['cache_refs']):>13,} "
          f"{d['misses_per_lookup']:>11.4f} {d['l1d_per_lookup']:>10.4f} "
          f"{d['dtlb_per_lookup']:>11.4f} {d['ipc']:>7.4f} {d['miss_rate_pct']:>9.2f}%")

print()
print("=" * 80)
print("TABLE 5: Cracker Throughput -- mean MH/s (stddev), 3 runs per cell")
print("=" * 80)
print(f"{'Impl':<10}", end="")
for t in [1, 2, 4, 8]:
    print(f"  {'%d thr' % t:>14}", end="")
print()
print("-" * 80)
for impl in IMPLS:
    print(f"{impl:<10}", end="")
...
print("=" * 80)
print(f"{'Impl':<10} {'Mean (s)':>10} {'Stddev (s)':>12} {'Min (s)':>9} {'Max (s)':>9} {'vs tinyptr':>12}")
print("-" * 80)
base = hyperfine["tinyptr"]["mean"]
for impl in IMPLS:
    d = hyperfine[impl]
    print(f"{impl:<10} {d['mean']:>10.3f} {d['stddev']:>12.3f} "
          f"{d['min']:>9.3f} {d['max']:>9.3f} {d['mean']/base:>11.2f}x")
