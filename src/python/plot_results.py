...
def parse_benchmark_csv(path: Path) -> list[dict]:
    # returns list of {name, real_time_ns, items_per_second} dicts
    rows = []
...
def parse_perf_txt(path: Path) -> list[dict]:
    # returns list of {impl, cache_misses, l1_misses, cycles, wall_time_s} dicts
...
def parse_thread_scaling_txt(path: Path) -> list[dict]:
    # returns list of {threads, runs, median_ms} dicts, one per thread count
...
def parse_cracker_comparison_txt(path: Path) -> dict[str, list[dict]]:
    # returns {impl_key: [{threads, runs, median_ms}, ...]} for each implementation
...
def plot_lookup_throughput(rows: list[dict], out_path: Path) -> None:
    # bar chart: ns per lookup for each implementation
    name_map = {
    ...
    ax.bar_label(bars, fmt="%.1f ns", padding=4, fontsize=10)
...
def plot_cache_misses(perf_rows: list[dict], out_path: Path) -> None:
    # grouped bar chart: LLC and L1 cache misses per implementation
    impls = [r["impl"] for r in perf_rows]
    llc   = [r.get("cache-misses", 0) / 1e6 for r in perf_rows]
    ...
    ax.set_ylabel("Cache misses (millions, lower is better)")
...
def plot_thread_scaling(scaling: list[dict], out_path: Path) -> None:
    # line chart: tinyptr crack time vs thread count with individual run scatter
    ...
    ax.set_ylabel("Crack time (ms, lower is better)")
...
def plot_cracker_comparison(comparison: dict[str, list[dict]], out_path: Path) -> None:
    # line chart: crack time vs thread count for all three implementations
